...
**Schema**:
The compiled JSON Schema (2020-12 or Draft 7) that drives the form. Loaded from a **Schema
hint**, a CLI argument, or a user choice.
...
group**, **Variant group**, **Tuple group**, **Unknown group**, **Cyclic stub**.
_Avoid_: Element, control, row.

**Field**:
A **Form node** that holds one settable scalar value. Carries a **Widget**, a **Presence**, and
its display metadata (title, description, default, unit, deprecation).
...
**Repeat group**:
A **Form node** for a Schema `array` of objects — the `+ add another server` case. Children are
index-addressed; its add and remove affordances are gated by `minItems` / `maxItems`.
...
byte-for-byte and surfaced with a **Violation**; never rewritten and never dropped, because a
hand-written config file that predates the Schema must still be openable.
...
node-exists/node-absent pair. Exactly one of **Absent**, **Set**, or **Invalid**.
...
**Setter intent**:
One member of confyg's closed command set (`SetValue`, `Unset`, `AddRepeatItem`,
`RemoveRepeatItem`, `MoveRepeatItem`, `AddMapEntry`, `RenameMapKey`, `RemoveMapEntry`,
`SelectVariant`, `ToggleGroup`, `GenerateTemplate`, `LoadSchema`, `Undo`, `Redo`). Every intent
is Schema-gated: the **Schema** decides whether it is offered and what it may write.
...
is precisely the notation freedom confyg drops relative to confy.
